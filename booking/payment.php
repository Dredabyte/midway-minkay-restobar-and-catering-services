...
$guest->company        = $_SESSION['company'];      
$guest->company_address       = $_SESSION['caddress'];        
$guest->terms          = 1;    
...
$_SESSION['guest_id'] =   $lastguest;
    ...
    for ($i=0; $i < $count_cart  ; $i++) { 
            ...
            $reservation->trans_date         = date('Y-m-d h:i:s'); 
      ...
      $sql = "INSERT INTO `payment` (`trans_date`,`confirmation_code`,`p_qty`, `guest_id`, `price`,`msg_view`,`status`)
       VALUES ('" .date('Y-m-d h:i:s')."','" . $_SESSION['confirmation'] ."',".$item."," . $_SESSION['guest_id'] . ",".$tot.",0,'Pending')" ;
